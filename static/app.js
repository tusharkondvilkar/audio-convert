...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const generatePlay = byId("generate-play");
  const generateDownload = byId("generate-download");
  const previewButton = byId("voice-preview");
  const previewIcon = byId("preview-icon");
  const samplePlayer = byId("sample-player");
  ...
  previewButton.addEventListener("click", async () => {
    if (samplePlayer.src && !samplePlayer.paused) {
      samplePlayer.pause();
    ...
    previewIcon.textContent = "...";
    message.textContent = "Loading voice sample...";
    try {
      const response = await fetch("/api/generate", {
        method: "POST",
        cache: "no-store",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: "Hello, welcome to HRMantra AI Audio Studio. This is a sample of my voice.", voice: voice.value, speed: speed.value, format: "mp3" }),
      });
      if (!response.ok) throw new Error("The voice sample could not be generated.");
      samplePlayer.src = URL.createObjectURL(await response.blob());
      await samplePlayer.play();
      previewIcon.textContent = "❚❚";
      message.textContent = "Playing the selected voice sample.";
    } catch (error) {
      previewIcon.textContent = "▶";
      message.textContent = error instanceof Error ? error.message : "Voice sample failed.";
    } finally {
      previewButton.disabled = false;
    }
  });
  samplePlayer.addEventListener("ended", () => { previewIcon.textContent = "▶"; message.textContent = ""; });
  voice.addEventListener("change", () => { samplePlayer.pause(); samplePlayer.removeAttribute("src"); previewIcon.textContent = "▶"; });

  async function createAudio(downloadImmediately) {
    message.textContent = "";
    result.hidden = true;
    ...
    generatePlay.disabled = true;
    generateDownload.disabled = true;
    ...
    try {
      ...
      if (downloadImmediately) {
        download.click();
        message.textContent = "Your download has started. You can also play the audio below.";
      } else {
        player.play().catch(() => {});
        message.textContent = "Audio is ready. Use Download later whenever you are ready.";
      }
    } catch (error) {
      stopProgress(false);
      message.textContent = error instanceof Error ? error.message : "Audio generation failed.";
    } finally {
      generatePlay.disabled = false;
      generateDownload.disabled = false;
      buttonText.textContent = "Generate & Download";
    }
  }

  generatePlay.addEventListener("click", () => createAudio(false));
  generateDownload.addEventListener("click", () => createAudio(true));
});
